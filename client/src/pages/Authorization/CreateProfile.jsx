...
const CreateProfile = () => {
    ...
    return <>
        <div className="create-profile-container flex flex-col">
            <div className="create-profile-nav-container p-10">
...
101 -25 18 -70 15 -93 -8z"/>
            ...
            <div className="create-profile-form-container self-center flex flex-col px-10 gap-y-10">
                <div className="heading-container flex flex-col gap-y-4">
                ...
                <form className="flex flex-col gap-y-4 mb-10">
                    <h2 className="text-xl font-bold">Add an avatar</h2>
                    <div className="image-input-container flex gap-x-10">
                        <div className={`preview-of-image-input border border-2 rounded-full text-center ${!isImageUploaded ? "border-dashed" : "border-0"}`}>
                            <img src={imagePreview} alt="image-preview" className="w-40 h-40 rounded-full object-cover" />
                        </div>
                        <div className="image-input-field">
                            <label htmlFor="profile" className="font-semibold border p-2 rounded-lg">Choose image</label>
                            <input type="file" onChange={handleImageChange} name="profile" id="profile" className="opacity-0 w-0" />

                            <p className="text-gray-500 mt-4">
                               <span className="text-xl">&gt;</span> Or choose one of our defaults
                            </p>
                        </div>
                    </div>

                    <div className="location-input-field flex flex-col gap-y-4 mt-10">
                        <label htmlFor="location" className="text-xl font-bold">Add your location</label>
                        <input type="text" name="location" id="location" className="outline-0 border-b border-gray-500 placeholder:text-gray-500 py-2" placeholder="Enter a location" />
                    </div>

                    <button className={`w-56 mt-10 px-4 py-2 rounded-md ${!isImageUploaded ? "bg-pink-200" : "bg-pink-500"} text-white font-bold`}>
                        Next
                    </button>

                    <p className={`text-gray-500 px-10 ${!isImageUploaded && "hidden"}`}>or press <span className="font-bold">RETURN</span></p>
                </form>
            </div>
        </div>
    </>;
